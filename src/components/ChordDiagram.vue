  ...
  <div class="chord-diagram">
    <div class="fretboard-row" v-for="(string, i) in strings" :key="i">
      ...
      >
        ...
        <div
          v-if="i === strings.length - 1 && barre && barre.fret === fret"
          :class="`barre ${barreClass}`"
        ></div>

        <div
          v-if="i === strings.length - 1"
          :class="{
            'fret-number': true,
...
export default {
  ...
  computed: {
    strings() {
      ...
      const strings =
        (value && value.map(s => s.replace(/\d/g, ""))) || label.split("");
  ...
  &:not(.is-over-barre) {
    .wrapper {
      ...
      &::before {
        ...
        transform: rotate(
          45deg
        ); // rotate to prevent sub pixel rounding squashing
      }
    }
  }
